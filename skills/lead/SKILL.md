...
2. Apply the Delegation Gate. For any meaningful work that changes project deliverables — source, tests, configuration, documentation, assets, or generated output — a worker must own the implementation. Root and Domain Leads do not implement it themselves. They may only coordinate, inspect read-only evidence, write `.orca-team` coordination records, prepare a research/contract brief, verify, and report. Group related tiny edits under one worker rather than making a worker per line. A pure status, clarification, policy, or one-sentence answer needs no worker.
...
For meaningful project work, the Lead must dispatch or reuse a worker; it must not quietly implement the task in its own Lead terminal. A worker may be created or reused only for a concrete `READY` task that has all of the following:
